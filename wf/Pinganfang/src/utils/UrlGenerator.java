...
import base.InterfaceParam;
import base.InterfaceTestEntity;
...
public class UrlGenerator {
	...
	public String doTest(boolean isRandom,String type,String agreement){
		String result=doTest(isRandom,type,testApi,interfaceTest,agreement);
	...
	private String doTest(String type,String testApi, InterfaceTestEntity interfaceTest,String agreement) {
		String result="";
		if(null!=interfaceTest.getPreExcution()&&!interfaceTest.getPreExcution().equals("")){
			result=doTest(false, "post",testApi,interfaceTest,agreement);
		}
		
		if(type.equalsIgnoreCase("post")){
				try {
				
					result=	doPost(false,testApi,interfaceTest,agreement);
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
	...
	public String doTest(boolean isRandom,String type,String testApi, InterfaceTestEntity interfaceTest,String agreement){
		...
		if(null!=interfaceTest.getPreExcution()&&interfaceTest.getPreExcution().length()>1){
			...
			try {
				...
				doc = saxReader.read(new File(GlobalInfo.rootPath + sp + "cases"
			...
			String sendAgreement=caseElem.attributeValue("agreement");
			String testApiNew=caseApiElement.attributeValue("api");
			String realApiUrl="";
			if(testApiNew.contains("pinganfang.com")){
			...
			interfaceTestEntity.setMethod(e.attributeValue("method"));
			interfaceTestEntity.setPreExcution(e.attributeValue("preExcution"));
			result=doTest(sendType,testApiNew,interfaceTestEntity,sendAgreement);
		...
		result=doTest(type,testApi,interfaceTest,agreement);
		return result;
	}
	
	public String doPost(boolean isRandom,String testApi, InterfaceTestEntity interfaceTest,String agreement) throws Exception {
		String strResult="";
		if(null!=agreement&&!agreement.equals("")&&agreement.equalsIgnoreCase("jsonrpc")){		
			url = testApi;
			String paramString="";
			List<InterfaceParam> parmsList=interfaceTest.getInterfaceParams();
			for (InterfaceParam inParam : parmsList) {
				String paraValue ="";
				paraValue = getVarible(inParam.getValue());
				
				if (paraValue!=null&&paraValue.length()>0) {
					if(paraValue.charAt(0)=='{'||paraValue.charAt(0)=='['){
						paramString+=paraValue+",";
					}
					else{
						paramString+="\""+paraValue+"\""+",";
					}
			...
			String jsonString="{\"jsonrpc\":\"2.0\",\"method\":\""+interfaceTest.getMethod()+"\",\"params\":["+paramString+"]}";
...
//				}
	...
	private String getCookie(HttpResponse response) {
		...
		for (Header location : headers) {
			...
			if (location.getName().equalsIgnoreCase("location")||location.getName().equalsIgnoreCase("Set-Cookie")) {
				for (String cookie : cookieList) {
					// if(cookie.startsWith("guid=")||cookie.startsWith("per_")||cookie.startsWith("cityid")){
	...
	public String getVarible(String varibleName) {
		...
		if (varibleName.startsWith("$")) {
			...
			if (varibleName.contains(".")) {
				String dd=varibleName
				.substring(1);
				String fdfdfd=dd.split("\\.")[1];
